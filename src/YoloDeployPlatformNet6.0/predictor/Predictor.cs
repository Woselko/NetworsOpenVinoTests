                ...
                case EngineType.OpenCV:
                    {
                        ...
                        for (int i = 0; i < num; i++)
                        {
                            mats[i] = Mat.FromPixelData(rows, cols, MatType.CV_32FC1, Marshal.UnsafeAddrOfPinnedArrayElement(input_data, i * rows * cols));
                                //new Mat(rows, cols, MatType.CV_32FC1, Marshal.UnsafeAddrOfPinnedArrayElement(input_data , i * rows * cols));
                        }
                        Cv2.Merge(mats, im);
                        ...
                        foreach (var name in output_names) 
                        {
                            ...
                            Mat result_mat_to_float = Mat.FromPixelData(output_sizes[index][1], output_sizes[index][2], MatType.CV_32F, result_mat.Data);
                           //new Mat(output_sizes[index][1], output_sizes[index][2], MatType.CV_32F, result_mat.Data);
                            int sum = 1;
